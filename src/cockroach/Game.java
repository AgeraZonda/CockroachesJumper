...
public class Game extends Canvas implements Runnable { // Khai báo biến
	...
	public Game() // Hàm Constructor
	{
		...
		fl2 = new Floor(200 + 400 - 85 + r.nextInt(170), HEIGHT - 200 - 85 + r.nextInt(170)); // Khai báo các cột để
																								// nhảy
		...
		this.addMouseListener(new MouseAdapter() // thêm sự kiện cho nút retry
		{
			public void mousePressed(MouseEvent e) {
				int mx = e.getX();
				int my = e.getY();
				if (mouseOver(mx, my, 170 + 300, 150 + 200, 120, 64) && stage == 4) // chuột click ở trong vùng nút
																					// retry
				{
					stage = 1;
				}
				...
				if (mouseOver(mx, my, 350, 400, 100, 50) && stage == 3) // chuột click ở trong vùng nút retry
				{
					...
					go.sc(sc);
					fl1 = new Floor(200, HEIGHT - 200);
					fl2 = new Floor(200 + 400 - 85 + r.nextInt(170), HEIGHT - 200 - 85 + r.nextInt(170)); // Khai báo
																											// các cột
																											// để nhảy
					fl3 = new Floor(200 + 800 - 85 + r.nextInt(170), HEIGHT - 200 - 85 + r.nextInt(170));
					fl4 = new Floor(200 + 1200 - 85 + r.nextInt(170), HEIGHT - 200 - 85 + r.nextInt(170));
	...
	private void tick() // Hàm xử Lý sau mỗi Khung Hình
	{
		...
		if (stage == 2) {
			...
			Line2D topln = new Line2D.Float(currentFloor.getX() - 20, currentFloor.getY() - 5, currentFloor.getX() + 90,
					currentFloor.getY() - 5);
			if ((p.getX() >= fl1.getX() && p.getX() <= fl1.getX() + 100 && topln.intersects(a) && p.getVelY() > 0)
					|| (p.getX() >= fl2.getX() && p.getX() <= fl2.getX() + 100 && topln.intersects(a)
							&& p.getVelY() > 0)
					|| (p.getX() >= fl3.getX() && p.getX() <= fl3.getX() + 100 && topln.intersects(a)
							&& p.getVelY() > 0)
					|| (p.getX() >= fl4.getX() && p.getX() <= fl4.getX() + 100 && topln.intersects(a)
							&& p.getVelY() > 0)) // hàm dừng nhân vật khi va chạm vào cột
			{
				p.setVelY(0);
				if (keyboard.getIsSpace()) {
					bar.setHeath(100);
	...
	private void render() // Hàm render hình ảnh game
	{
		...
		if (stage == 2 && keyboard.getIspress() == 1 && keyboard.getIsSpace()==false) {
			holding++;
			if (holding >= 5500)
				holding = 5500;
			g.setColor(new Color(0, 85, 149));
